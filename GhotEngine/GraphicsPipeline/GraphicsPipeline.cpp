...
void GraphicsPipeline::CreateRootSignature(Microsoft::WRL::ComPtr <ID3D12Device> device, D3D12_ROOT_SIGNATURE_DESC& descriptionRootSignature, Property& property) {
	...
	// シリアライズしてバイナリにする
	HRESULT hr_;
	hr_ = D3D12SerializeRootSignature(&descriptionRootSignature, D3D_ROOT_SIGNATURE_VERSION_1, &property.signatureBlob_, &property.errorBlob_);
	if (FAILED(hr_)) {
...
Property GraphicsPipeline::CreateSkinng(Microsoft::WRL::ComPtr<ID3D12Device> device, const std::wstring& shaderName){
	Property property;
	...
	pixelShaderBlob_ = compile_->CompileShader(L"GhotEngine/ShaderFile/Object3d.PS.hlsl", L"ps_6_0", dxcUtils, dxcCompiler, includeHandler);
	assert(pixelShaderBlob_ != nullptr);
...
Property GraphicsPipeline::CreateCopyImage(Microsoft::WRL::ComPtr<ID3D12Device> device, const std::wstring& shaderName)
{
	Property property;
	...
	D3D12_DEPTH_STENCIL_DESC depthStencilDesc{};
	// Depthの機能を有効化する
	depthStencilDesc.DepthEnable = true;//比較するのでDepth自体は有効
